...
/*UART1 new declaration
uint8_t numberOfButton = 1, numberOfDevice = 1;
uint8_t count1 = 0, flagReceiveAllDataFromSystem = 0;
unsigned char tempdataReceiveFromSystem, dataReceiveFromSystem[11], dataSendtoSystem[11];
uint8_t countSensor = 1 ,temperature = 0, humid = 0, gasDetection = 0, humanDetection = 0, power = 0;
*/
/*UART2 Internet Original 
...
//UART2 Internet New
uint8_t count2 = 0, flagReceiveAllDataFromInternet = 0;
unsigned char dataReceiveFromInternet[18], devicesStateUpdateToInternet[18] = "S0000000000000000E";
unsigned char dataFromSensorToInternet[11] = "S03C01000TE";
unsigned char dataFromPowerToInternet[11] = "S03D04000PE";
unsigned char tempdataReceiveFromInternet;
...
/*My code-----------------------------------------------------------------------------
	System = Slave. eg. answerSystem =>answer Slave
*/
//test RS485 connection
void receiveRS485fromSystem(void);
...
int main(void)
{
  ...
  {
		//My code-----------------------------------------------------------------------------//
		//test transmit to rs485

		//sendRS485toSystem();		//this works
		//HAL_Delay (1000);
		
		//use 2 RS485 to test
		//SET pin UART A to 1, UART B must be 0 to receive data
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	if(huart->Instance==huart1.Instance){
		HAL_UART_Receive_IT(&huart1,&tempdataReceiveFromSystem,1);		//data nhan lien tuc 1 byte, pass vao bien tempdataReceiveFromSystem
		if(tempdataReceiveFromSystem == 'S'){													
			count1 = 0;																									//increase by 1, stop until nothing is sent to from Slave
			dataReceiveFromSystem[count1] = tempdataReceiveFromSystem;	//1st loop dataReceiveFromSystem = S, tiep tuc count len va xet vao vong lap setIDforButton,Device 
			count1++;
		}
...
//-------------------------------------------------------------------------------------//
//My code-----------------------------------------------------------------------------//
//see http://blockofcodes.blogspot.com/2013/07/how-to-convert-string-to-integer-in-c.html
uint8_t BCD2DEC(uint8_t data){
...
uint8_t convertNumToStringLSB(uint8_t number){
	return (number%10)+48;																	//divide and get remainder
}
...
//send through PB5 pin
//send on FIRST UART
void sendRS485toSystem(void){
	//SET PB5, PD12 to 1 to enter transmitting mode
...
/*for testing, config another UART
void receiveRS485fromSystem(void){
	//While PB5 is 1, Pxx must be 0 to receive (use 2 RS485)
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5,GPIO_PIN_SET);
...
*/
//Slave Button ask for ID => setIDforButton
//Could be deleted because not functioning

void setIDforButton(void){
	static uint8_t i = 0;
	for(i = 0; i<(dataReceiveFromSystem[9]-48);i++){								//convert string to interger. notice LESS than
		dataSendtoSystem[0]  = 'S';																		//Begin transmission
		dataSendtoSystem[1]  = '1';																		//Master send to Slave
		dataSendtoSystem[2]  = '2';																		//"2" la lenh "set ID"
		//Sau B la id gom 2 byte cua 1 button tren 1 board Slave Button
		...
		//Sau D la id gom 2 byte cua 1 RELAY ung voi tung ID cua button da set phia tren, tren 1 board Slave Device
		...
		dataSendtoSystem[9]  = convertNumToStringLSB(i+1);						//max i=9 -> (i+1 =10)%10 get remainder=0 => decimal value=48=> string obtained = "0"	
		dataSendtoSystem[10] = 'E';																		//End frame => S12 Bxx Dxx 0E
		numberOfButton++;
		sendRS485toSystem();																					//send Data back to Slave to complete "set ID" command
		HAL_Delay(400);
	}
}

/*Set ID cho 1 board Slave Device (gom 3 relay)
but Device has been set for fixed ID
Could be deleted because not functioning
*/
void setIDforDevice(void){
	static uint8_t i = 0;
...
/*receive data from Slave Button to control Slave Device
	by sending frame to Slave Device
*/
void controlDevice(void){
		dataSendtoSystem[0]  = 'S';
		dataSendtoSystem[1]  = '1';																		//Master send to Slave
		dataSendtoSystem[2]  = '0';																		//"0" la lenh "control"
	//Sau D la ID cua device se dc control
		...
		dataSendtoSystem[7]  = '0';
		dataSendtoSystem[8]  = '0';
	//gia tri nhan ve cua dataReceiveFromSystem[9] la M,H,T,P, etc de nhan du lieu tu sensor va xu ly tiep (gui qua internet, etc)
		dataSendtoSystem[9]  = dataReceiveFromSystem[9];
		dataSendtoSystem[10] = 'E';
		sendRS485toSystem();																					//send command frame to Slave to complete "Control" command
}
...
void updateDevicesState(void){
	...
	devicesState[i] = dataSendtoSystem[9];															//device state on off depends on this bit
}
...
//Function to response requests from Slave
void answerSystem(void){
	//control device
	if(dataReceiveFromSystem[2] == '0'){
		controlDevice();
		updateDevicesState();										//Update to by dataSendtoSystem[9] (data send to Slave)
		updateStateToInternet();								//update to control by Internet
	}
/************************************
	set ID but HAVE TO CHECK THIS AGAIN
	conflict with code for Slave
	Could be deleted because not functioning
	...
	***********************************/
	...
	if(dataReceiveFromSystem[2] == '3'){
		...
		//dataReceiveFromSystem[9] la cac lenh doc gia tri tu sensor.
		//H la do am
		...
		//P doc gia tri cong suat tieu thu
...
void checkSetTime(void){
	uint8_t i;
	for(i=1;i<11;i++){
		//Condition to turn ON device
		if(setHourOn[i] > 0 || setMinOn[i] > 0){
				if(setHourOn[i] == hour && setMinOn[i] == minute){
						setHourOn[i] = 0;
						setMinOn[i] = 0;
						dataSendtoSystem[0]  = 'S';													//enter transmit mode
						dataSendtoSystem[1]  = '1';													//Master send to Slave
						dataSendtoSystem[2]  = '0';				
						dataSendtoSystem[3]  = 'D';														
						dataSendtoSystem[4]  = convertNumToStringMSB(i);		//1st ID of Device Dxx
						dataSendtoSystem[5]  = convertNumToStringLSB(i);		//2nd ID of Device Dxx
						dataSendtoSystem[6]  = '0';
						dataSendtoSystem[7]  = '0';
						dataSendtoSystem[8]  = '0';
						dataSendtoSystem[9]  = '1';													//turn ON device
						dataSendtoSystem[10] = 'E';
						sendRS485toSystem();																//send above frame to Slave
						HAL_GPIO_WritePin(GPIOD,GPIO_PIN_13,GPIO_PIN_SET);
						updateDevicesState();
						updateStateToInternet();
				}
		}
		//Condition to turn OFF device
...
void Call(void){
	HAL_UART_Transmit_IT(&huart3,(uint8_t *)"ATD0974255367;\r\n",16); // TODO: 0974255367, Mua sim khac
	HAL_Delay(15000);
...
void sendMessage(char *s,int length){
	uint8_t send = 0x1A;
	HAL_UART_Transmit_IT(&huart3,(uint8_t *)"AT+CMGS=\"0974255367\"\r\n",22);
	HAL_Delay(3000);
...
void processDetailSms (void){
	if(strcmp((const char *)smsDetail,"Turn on device 1") == 0){
		devicesState[1]='1';
		strcpy((char *)dataSendtoSystem,"S10D010001E");		//bit 1 truoc bit E turn on, Master to Slave, turn on device 01
		sendRS485toSystem();
		updateStateToInternet();
	...
	if(strcmp((const char *)smsDetail,"Turn off device 1") == 0){
		devicesState[1]='0';
		strcpy((char *)dataSendtoSystem,"S10D010000E");		//bit 0 truoc bit E turn off, Master to Slave, turn off device 01
		sendRS485toSystem();
		updateStateToInternet();

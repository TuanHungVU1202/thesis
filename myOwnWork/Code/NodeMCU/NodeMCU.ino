...
//must be same port as in Application server
const int httpPort = 2111;
int updateFlag = 0;
...
void loop() {
    ...
    if(updateFlag == 1){
      ...
      //try to change below line to descrease delay time
...
//GET deviceState (device state) from responsed JSON page
void readJSONFromStatePage (void) {
    if (!client.connect(host, httpPort)) { 
        //Serial.println("Khong ket noi duoc");
        return;
    ...
    //after receive state from JSON page
    //function controlling device 
...
//send control info to system to turn ON/OFF devices
void sendStateFromInternetToSystem(void){
...
//send device State to Application Server
void controlDevice(String device, String state){
    //req.query.deviceX in Application Server js will get info from here
    ...
    url += state;
    if (!client.connect(host, httpPort)) { 
      Serial.println("Error with connection. Please check your IP or opened port again");
      return;
    }
...
//Use req.query of Nodejs Express module to get data from route (after "?" and "=")
void sendDataFromSensorToInternet(String type, int value){
    String url;
    if(type == "Temp"){
...
//process data received from System for further commands
void processDataFromSystem(void){
